...
internal sealed class DiscordRpcClient : IAsyncDisposable
{
    ...
    private void HandlePayload(JsonElement root)
    {
        ...
        switch (evt)
        {
            ...
            case "VOICE_STATE_UPDATE":
                DiscordVoiceMember? member = ParseVoiceMember(dataElement);
            ...
            case "VOICE_STATE_DELETE":
                DiscordVoiceMember? removed = ParseVoiceMember(dataElement);
                string? removedId = removed?.UserId ?? GetString(dataElement, "user_id");
    ...
    private static DiscordVoiceMember? ParseVoiceMember(JsonElement state)
    {
        if (state.ValueKind != JsonValueKind.Object ||
            !state.TryGetProperty("user", out JsonElement user) ||
        ...
        string displayName = GetString(state, "nick") ??
                             GetString(user, "global_name") ??
                             GetString(user, "username") ??
                             "Discord user";
        return new DiscordVoiceMember(
            userId,
            displayName,
            GetString(user, "avatar"),
            user.TryGetProperty("bot", out JsonElement bot) && bot.ValueKind == JsonValueKind.True);
    }
